...
const EXTENSIONS = ["mp4", "m4s", "m4v", "mov", "mkv", "webm", "avi", "flv"];

function MediaScanner() {}

MediaScanner.Log = function () {
    const env = process.env.NODE_ENV;
    (!env || env !== "production") && console.log([...arguments].join(" "));
};
...
MediaScanner.ScanLibrary = async (lib) => {
    MediaScanner.Log("scanning library", lib.title);
    for await (const file of readdirp(lib.path)) {
        ...
        if (!EXTENSIONS.includes(ext.toLowerCase())) {
            MediaScanner.Log("skipping", file.basename, "wrong extension");
            continue;
        }
...
MediaScanner.ScanMovie = async (lib, { fullPath, basename }) => {
    ...
    MediaScanner.Log("scanned", data.title);
...
MediaScanner.ScanSeries = async (lib, { fullPath, basename }) => {
    ...
    MediaScanner.Log("scanned", data.title, "S" + season, "E" + episode);
...
MediaScanner.ScanGeneric = async (lib, { fullPath, basename }) => {
    ...
    MediaScanner.Log("scanned", title);
